...
function AddMealSideBar(props) {
  const { title, setAddMeal, addMealCategory, editMealCategory, menuID, companyID, addOREditMeal } = props;
  ...
  const handleMealEdit = (values, action) => {
    ...
    editMealCategory(companyID, menuID, parseInt(categoryID), mealID, mealData);
    setAddMeal(null);
  };
